...
# SQLite exports must never treat WAL/SHM sidecars as standalone databases,
# and retention must match the eight-digit YYYYMMDD timestamp generated by the
# backup jobs.
if grep -q -- "-name '\*.db-\*'\|-name '\*.sqlite-\*'" \
  "$repo_root/modules/storage/core.nix"; then
  echo "SQLite sidecar patterns must not be used for sqlite3 export discovery" >&2
  exit 1
fi
grep -q '????????T??????Z)' "$repo_root/modules/storage/core.nix"
