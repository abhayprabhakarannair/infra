...
in {
  home-manager.users.abhay = {
    ...
    programs.bash.shellAliases = {
      ll = "ls -larth";
      nrb = "nixos-rebuild build --flake ~/Projects/infra#${config.networking.hostName}";
      nrs = "nixos-rebuild switch --flake ~/Projects/infra#${config.networking.hostName}";
      ve = "nvim .";
    };
    ...
    home.file.".config/firefox/startpage.html" = {
      force = true;
      text = ''
      <!doctype html>
      <html lang="en">
    ...
    # Home Manager normally links this generated page into the Nix store.
    # Materialize it in the persistent home directory so Firefox keeps the
    # friendly /home path in the address bar.
    home.activation.materializeFirefoxStartPage = inputs.home-manager.lib.hm.dag.entryAfter ["linkGeneration"] ''
      firefoxStartPage="$HOME/.config/firefox/startpage.html"
      if [ -L "$firefoxStartPage" ] && [[ "$(readlink "$firefoxStartPage")" == /nix/store/* ]]; then
        firefoxStartPageTmp="$firefoxStartPage.tmp"
        cp --dereference "$firefoxStartPage" "$firefoxStartPageTmp"
        mv "$firefoxStartPageTmp" "$firefoxStartPage"
      fi
    '';
